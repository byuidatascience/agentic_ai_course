...
| `lessons/ practice/ preps/ notebooks/` | day-placed instances | comment blocks; day from filename `unit_day` |
...
- **Type** defaults to the folder (`preps/`→prep_reading, `practice/`→practice,
  `notebooks/`→example, `lessons/`→in_class); the comment wins if present.
